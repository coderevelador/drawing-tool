...
export const usePDFHandler = () => {
  ...
  const setPdfImage = useCanvasStore.getState().setPdfImage;

  // Render a given page number onto the canvas
  const renderPage = async (
    pageNum,
    pdf = useCanvasStore.getState().pdfDoc
  ) => {
    ...
    image.onload = () => {
      setPdfImage(image); // ✅ store it in Zustand
      canvasEngine.redrawCanvas(); // trigger re-render
    };
  };
  ...
  const handleFileUpload = useCallback(
    ...
    [canvasEngine, setPdfDoc, setTotalPages, setCurrentPage, setPdfLoading]
  );
