...
export class StickyNoteTool extends BaseTool {
  ...
  _applyEditorStyles(ed) {
    ...
    ed.style.boxShadow = this.state.shadow ? "0 6px 22px rgba(0,0,0,0.18)" : "none";
  ...
  _makePalette(host, editorDiv) {
    ...
    const colors = ["#FFF9B1","#FFD6A5","#FDFFB6","#CAFFBF","#A0E7E5","#BDB2FF","#FFC6FF","#FFADAD","#FFFFFF","#F1F5F9"];
    colors.forEach(c => {
      const b = document.createElement("button");
      Object.assign(b.style, {
        width: "24px", height: "24px", borderRadius: "6px",
        border: "2px solid #0b0b0b", background: c, cursor: "pointer"
      });
      b.onclick = () => {
        this.state.bg = c;
    ...
    row2.style.display = "flex"; row2.style.gap = "8px"; row2.style.alignItems = "center";
    const opLabel = document.createElement("span"); opLabel.textContent = "Opacity";
    const op = document.createElement("input");
    Object.assign(op, { type:"range", min:"0.5", max:"1", step:"0.05", value:String(this.state.opacity) });
    op.oninput = () => { this.state.opacity = parseFloat(op.value); this._applyEditorStyles(editorDiv); };
    ...
    minus.textContent = "−"; minus.style.padding = "4px 8px";
    minus.onclick = () => { this.state.fontSize = Math.max(10, this.state.fontSize - 1); this._applyEditorStyles(editorDiv); };
    const plus = document.createElement("button");
    plus.textContent = "+"; plus.style.padding = "4px 8px";
    plus.onclick = () => { this.state.fontSize = Math.min(48, this.state.fontSize + 1); this._applyEditorStyles(editorDiv); };
    const color = document.createElement("input");
    color.type = "color"; color.value = this.state.color;
    color.oninput = () => { this.state.color = color.value; this._applyEditorStyles(editorDiv); };
  ...
  _createEditor(engine, rect) {
    ...
    const sref = (engine?.store && typeof engine.store.getState === "function") ? engine.store : useCanvasStore;
    const st = sref.getState();
    if (st?.color) this.state.color = st.color;
    if (st?.lineWidth) this.state.fontSize = Math.max(12, Math.round(st.lineWidth * 6));
  ...
  _finalize(engine, rect, ed, commit) {
    ...
    if (!commit || !text.trim()) {
      this.drawing = false;
      this.start = null; this.curr = null; this.activeRect = null;
      return;
    }
