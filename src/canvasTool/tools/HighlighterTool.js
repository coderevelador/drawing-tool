...
export class HighlighterTool extends BaseTool {
  ...
  _drawToBuffer() {
    ...
    // midpoint smoothing
    const m0x = (p_1.x + p0.x) / 2, m0y = (p_1.y + p0.y) / 2;
  ...
  onKeyDown(event, engine) {
    ...
    if (event.key.toLowerCase() === "o") {
      const v = parseFloat(window.prompt("Highlighter opacity (0..1):", String(this.opacity)));
      if (!Number.isNaN(v) && v >= 0 && v <= 1) this.opacity = v;
    }
    if (event.key.toLowerCase() === "m") {
      this.composite = (this.composite === "multiply") ? "source-over" : "multiply";
    }
