...
export class CalloutTool extends BaseTool {
  static inspector = [
    ...
    { group: "Text",  label: "Font family",type: "text",   path: "style.fontFamily" },
    { group: "Text",  label: "Text color", type: "color",  path: "style.textColor" },
  ...
  _fillPreview(ctx, s) {
    ...
    ctx.globalAlpha = prev * (typeof s.fillOpacity === "number" ? s.fillOpacity : 1);
    ctx.fill();
  ...
  _bubblePath(ctx, x, y, w, h, s) {
    ...
    const tailStartX = x + Math.max(0, Math.min(w, (s.tailOffset != null ? s.tailOffset : w * 0.7)));
  ...
  _openEditor(engine, rect, onCommit) {
    ...
    Object.assign(ta.style, {
      ...
      color: this._styleSnapshot.textColor || this._styleSnapshot.stroke || "#000",
