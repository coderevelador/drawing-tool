...
export class ArrowTool extends BaseTool {
  static inspector = [
    ...
    { group: "Style",  label: "Head size", type: "number", path: "style.headSize", min: 4, step: 1 },
  ...
  _drawArrowPreview(ctx, from, to, s) {
    ...
    // head (triangle)
    const headLen = Math.max(4, Number(s.headSize ?? Math.max(10, (s.lineWidth ?? 2) * 3)));
  ...
  onMouseDown(e, pos, engine) {
    ...
    this._styleSnapshot = {
      ...
      headSize: Number(style.headSize ?? Math.max(10, (style.lineWidth ?? 2) * 3)),
