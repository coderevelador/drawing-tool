...
export class CanvasEngine {
  ...
  handleCalloutClick(x, y) {
    const { callouts, setEditingCallout, setCalloutText } = this.store.getState();
  ...
  saveBackgroundSnapshot() {
    this.backgroundSnapshot = this.ctx.getImageData(0, 0, this.width, this.height);
  }
  ...
  redrawCanvas() {
    if (this.drawingSnapshot) {
      this.ctx.putImageData(this.drawingSnapshot, 0, 0);
    } else if (this.backgroundSnapshot) {
